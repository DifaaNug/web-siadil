...
const computePlacement = (
  ...
  const notEnoughSpaceBelow =
    triggerRect.bottom + MENU_OFFSET + menuHeight > viewportHeight - VIEWPORT_PADDING;
  const enoughSpaceAbove = triggerRect.top - MENU_OFFSET - menuHeight > VIEWPORT_PADDING;
...
export const ActionMenu = ({
  ...
  const [menuStyle, setMenuStyle] = useState<React.CSSProperties>(initialPosition.style);
  const [transformOrigin, setTransformOrigin] = useState<React.CSSProperties["transformOrigin"]>(
  ...
  const updateMenuPosition = useCallback(() => {
    ...
    setMenuStyle((prev) => {
      ...
      for (const key of keys) {
        if (prev[key as keyof React.CSSProperties] !== style[key as keyof React.CSSProperties]) {
          return style;
        }
      }
      return prev;
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          animation: isReady ? "fadeIn 0.12s cubic-bezier(0.16, 1, 0.3, 1) forwards" : undefined,
          transformOrigin,
        }}>
        <div className="p-3">
              ...
              return (
                <li key={index} style={{ animationDelay: `${index * 40}ms` }}>
                  <button
                    ...
                    style={{
                      animation: `fadeIn 0.2s cubic-bezier(0.16, 1, 0.3, 1) forwards ${index * 40}ms both`,
                    }}>
