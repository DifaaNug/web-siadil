...
export const useSelection = (
    ...
    const [infoPanelDocument, setInfoPanelDocument] = useState<Document | null>(null);
    ...
    const handleDocumentSelect = (docId: string, event?: React.MouseEvent) => {
        ...
        } else if (!isRightClick && event?.shiftKey && paginatedDocuments.length > 0) {
          const lastSelected = Array.from(selectedDocumentIds).pop();
          if (lastSelected) {
            const lastIndex = paginatedDocuments.findIndex((d) => d.id === lastSelected);
            const currentIndex = paginatedDocuments.findIndex((d) => d.id === docId);
